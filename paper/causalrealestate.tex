...
We have arranged what follows in the order a skeptical reader would want to work through it. The literatures on spatial confounding, on concept erasure, and on text as a treatment have grown up largely apart from one another, and Section~\ref{sec:related} brings them into contact, after which the estimand, the linear and nonlinear erasure pipeline, and the stability result that the later analysis leans on are developed in Section~\ref{sec:methods}. How the corpus was assembled, and audited for the duplication that scraped listings invite, is the business of Section~\ref{sec:data}, and the calibration study behind our inferential caution follows in Section~\ref{sec:sim-coverage}, where the influence-function interval is shown to undercover for an embedding treatment and a bootstrap over the pipeline is shown to repair it. The twelve-market evidence occupies Section~\ref{sec:results}, which moves in turn through the uniqueness and principal-component estimates, the rewriting intervention, the erasure and decomposition diagnostics, the sensitivity bounds, and the cross-method comparison, before Section~\ref{sec:discussion} closes by drawing the line, for practice, between what a predictive-only evaluation of text-augmented valuation can and cannot support.
