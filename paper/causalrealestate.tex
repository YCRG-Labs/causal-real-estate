...
The repair the simulation points to is a percentile bootstrap that resamples listings with replacement and refits the principal-component projection together with both nuisance models inside each resample, so that the variance the projection contributes is carried into the interval rather than assumed away. Applying it to the twelve-market panel is left to the revision this appendix motivates; the intervals reported in Section~\ref{sec:baur-12city} are influence-function intervals throughout, and no bootstrap interval is claimed for any market. The doubly-robust estimator that anchors the panel has no power against the continuous effect, because binarizing at the median principal-component norm discards the graded signal the partialling-out estimator targets, so we read its near-zero estimate as a calibration check on the inference machinery rather than as a competing estimate of the semantic effect.
