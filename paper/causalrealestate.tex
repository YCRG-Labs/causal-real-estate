...
\title{Deconfounding Location from Semantic Signals in Real Estate Valuation}
...
\renewcommand{\shorttitle}{Deconfounding Location and Semantics in Real Estate Valuation}
...
structured attributes, and the language reliably sharpens price predictions, yet
better prediction cannot say whether the text carries genuine property information
or merely relabels the neighborhood geography an agent's description encodes.
Treating the leading direction of a listing embedding as a high-dimensional
treatment, we estimate its effect on log listing price with a partially linear
double machine learning score and ask how much survives once location is removed.
Across twelve metropolitan markets the embedding provably encodes location,
recoverable by a held-out probe before a closed-form erasure and essentially not
after, yet holding the text fixed and adjusting for geography with a flexible
thin-plate basis leaves the per-standard-deviation price effect almost unmoved in
eleven markets, the exception being New York, whose language is the most
geographic of the twelve. Because the estimates are associational rather than
cleanly identified, and because the outcome is an agent-set asking price produced
together with the text, we bound their fragility with a sensitivity analysis and
are explicit about what the design does and does not settle. The contribution is a
disciplined protocol for measuring and deconfounding the price effect of a learned
text representation rather than a causal elasticity of price on language.
\end{abstract}
