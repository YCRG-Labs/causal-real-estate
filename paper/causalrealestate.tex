...
\begin{axis}[
    ...
    symbolic x coords={Boston,New York,San Francisco},
    xtick=data, xticklabel style={font=\small},
    ylabel={DML $\hat\theta$ on PC1 (per $\sigma$, log-price)},
    ymin=-0.30, ymax=0.32,
    title={(b) Per-city causal effect on PC1},
    title style={font=\normalsize},
...
\addplot coordinates {(Boston,-0.026) (New York,0.169) (San Francisco,-0.026)};
\addplot[only marks, mark=-, mark size=3pt, thick, black, forget plot]
    coordinates {(Boston,-0.151) (Boston,0.098) (New York,0.059) (New York,0.279) (San Francisco,-0.150) (San Francisco,0.098)};
\draw[thick,gray] (axis cs:Boston,-0.151) -- (axis cs:Boston,0.098);
\draw[thick,gray] (axis cs:New York,0.059) -- (axis cs:New York,0.279);
\draw[thick,gray] (axis cs:San Francisco,-0.150) -- (axis cs:San Francisco,0.098);
\node[font=\scriptsize, anchor=west] at (axis cs:Boston,-0.060) {$-0.026$};
\node[font=\scriptsize, anchor=west] at (axis cs:New York,0.200) {$+0.169$};
\node[font=\scriptsize, anchor=west] at (axis cs:San Francisco,-0.060) {$-0.026$};
\end{axis}
\end{tikzpicture}
\caption{The heterogeneity of causal text effects across markets. (a) Correlational $\Delta R^2$: text embeddings add 0.85--0.88 $R^2$ beyond location controls in all three cities. (b) DML estimate on the leading principal component with Ibragimov--M\"uller cluster-$t$ 95\% confidence intervals (whiskers). New York excludes zero at $+0.169$ per $\sigma$; San Francisco and Boston are null on this axis. The Wald test on equality of the three coefficients returns $\chi^2_2 = 7.97$, $p = 0.019$ under cheap-bootstrap standard errors. San Francisco's residual signal lives on a separate Shen-Ross uniqueness axis with effect $+0.130$ per $\sigma$, CI $[+0.011, +0.232]$ (Section~\ref{sec:replications}).}
...
\begin{axis}[
    ...
    ybar, bar width=14pt,
    symbolic x coords={NYC$\to$SF, NYC$\to$Bos, SF$\to$NYC, SF$\to$Bos, Bos$\to$NYC, Bos$\to$SF},
    xtick=data, xticklabel style={font=\scriptsize, rotate=20, anchor=east},
    ylabel={Transfer Spearman $\rho$},
    ymin=0, ymax=0.55,
    axis lines*=left,
    title={Cross-Market Rank Portability}, title style={font=\normalsize},
    enlarge x limits=0.10,
    every axis plot/.append style={draw=black!70},
]
\addplot[fill=blue!60!white] coordinates {(NYC$\to$SF,0.321) (NYC$\to$Bos,0.383) (SF$\to$NYC,0.335) (SF$\to$Bos,0.216) (Bos$\to$NYC,0.452) (Bos$\to$SF,0.219)};
\node[font=\scriptsize, anchor=south] at (axis cs:NYC$\to$SF,0.321) {0.32};
\node[font=\scriptsize, anchor=south] at (axis cs:NYC$\to$Bos,0.383) {0.38};
\node[font=\scriptsize, anchor=south] at (axis cs:SF$\to$NYC,0.335) {0.34};
\node[font=\scriptsize, anchor=south] at (axis cs:SF$\to$Bos,0.216) {0.22};
\node[font=\scriptsize, anchor=south] at (axis cs:Bos$\to$NYC,0.452) {0.45};
\node[font=\scriptsize, anchor=south] at (axis cs:Bos$\to$SF,0.219) {0.22};
\end{axis}
\end{tikzpicture}
\caption{Cross-market transfer test. Transfer Spearman $\rho$ averages $0.32$ across six pairs, indicating moderate rather than catastrophic rank portability. Raw transfer $R^2$ values are near zero across all six pairs (mean $-0.010$); the $R^2 < 0$ result in earlier versions of this paper was a duplication-overfit artifact and is not replicable on the deduplicated data.}
\label{fig:transfer}
\end{figure}
\FloatBarrier
