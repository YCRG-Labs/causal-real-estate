...
Each generation of housing-market technology has produced its own characteristic disparity between groups, and each disparity has called for its own audit. \citet{avenancio2022assessment} demonstrate on a panel of 118 million homes that Black and Hispanic owners face a 10--13\% higher property-tax burden for the same bundle of public services, with just over half of the inequality arising between neighbourhoods because assessments respond less to neighbourhood attributes than market prices do. A credit-side parallel runs through \citet{bartlett2022consumer} on the persistence of rate disparities through the FinTech transition and \citet{fuster2022predictably} on the way machine learning amplifies, rather than removes, between-group divergence in predicted default. \citet{neal2024racial} extend the pattern into automated valuation directly, documenting that AVM error remains larger in majority-Black tracts even after data and architectural improvements. What the literature has not yet asked, and what this paper takes up, is whether the natural-language component of a modern AVM produces causal mispricing that varies by market, and whether the operative residual axis on which the model acts shifts in a way that predictive-only audits cannot detect.

The substantive question is straightforward in form and surprisingly subtle in answer. Property descriptions are written by agents who possess complete knowledge of the property's location, neighbourhood, and local market conditions, so any embedding of those descriptions inherits at least some spatial signal whether or not the embedding model intends it. Recent work claims that text adds real semantic content beyond geographic controls, with reported gains of 0.85--0.88 in predictive $R^2$ from embeddings on top of location alone~\citep{shen2021information,baur2023automated,bin2020multisource,lorenz2023interpretable}. A plain test of that claim asks whether the per-$\sigma$ causal effect of an embedding is positive after standard spatial confounding adjustment. Running the test on a single market answers the question for that market and only that market, because the embedding's geometric structure, the available structured covariates, and the per-zip listing density all vary across U.S.\ metropolitan housing markets in ways that determine how much room remains for text to carry causal signal once spatial controls absorb the location-confounded component of variation.

Our register is causal fairness rather than predictive fairness, and the distinction matters for what an audit can see. Predictive-fairness audits compare model errors at fixed inputs and detect disparities that show up as miscalibration or differential accuracy. Causal-fairness audits, in the sense developed by \citet{kusner2017counterfactual}, \citet{madras2019fairness}, and \citet{plecko2024causal}, compare model contributions at fixed counterfactual inputs and detect disparities that operate through the partial causal effect of a model component on outcomes. NLP-augmented valuation creates the first kind of disparity wherever its inputs differ systematically across groups, and it creates the second kind wherever its operative residual axis differs systematically across markets. These two disparities require different audits because they live in different parts of the prediction.

Three reinforcing margins organise our contribution. A sin-$\Theta$ stability theorem for the leading principal component of an embedding under spatial confounding (Theorem~\ref{thm:eigengap}) identifies the eigengap of a Mat\'ern kernel-ridge smoother as the operative quantity governing alignment with the residual causal direction, and characterises the finite-sample regime in which the asymptotic eigenvalue rate of \citet{widom1963} binds for the Mat\'ern-$\nu$ class. Estimation uses cross-fitted ridge nuisances inside a partially-linear DML score, with Ibragimov--M\"uller self-normalised cluster-$t$ inference replacing the asymptotic-$z$ critical value that a Davis--Kahan stress simulation reveals to undercover at nominal 95\% confidence (Appendix~\ref{app:dk_sim}). On the replication side, the procedure recovers the Shen-Ross uniqueness effect on San Francisco at $+0.130$ per $\sigma$ within $0.7$ standard errors of her published Atlanta point estimate, isolates the leading sentence-BERT principal component's effect on New York at $+0.169$ per $\sigma$ on a heterogeneity test that rejects equality across cities at $\chi^2_2 = 7.97$, $p = 0.019$, and ships data and code for reuse on a deduplicated 1{,}044-listing corpus.

The remainder of the paper proceeds in conventional order. Section~\ref{sec:related} situates the contribution in the algorithmic-disparities literature for housing valuation alongside the hedonic and spatial-confounding traditions. Section~\ref{sec:methods} develops the partially-linear DML framework, the post-LEACE linear concept erasure pipeline with its iterative gradient-based polish, and the sin-$\Theta$ stability theorem. Section~\ref{sec:data} describes dataset construction and the deduplication audit. Per-city causal estimates, the heterogeneity test, the omnibus joint-null result on a residual subspace orthogonal to the dominant principal component, and the cell-density and confounder-density mechanisms that organise which market displays which effect appear in Section~\ref{sec:results}. Section~\ref{sec:discussion} closes with the fair-housing implications of the heterogeneity finding under \citet{plecko2024causal} causal fairness analysis.
...
\begin{axis}[
    ...
    ytick={-0.30,-0.20,-0.10,0,0.10},
    extra y ticks={0},
    extra y tick style={grid=major, grid style={black, dashed, thin}},
    extra y tick labels={},
    clip=true,
]
\draw[thin, gray] (axis cs:32,-0.290)  -- (axis cs:32, 0.090);
\draw[thin, gray] (axis cs:43,-0.290)  -- (axis cs:43, 0.090);
\draw[thin, gray] (axis cs:47,-0.290)  -- (axis cs:47, 0.090);
\draw[thin, gray] (axis cs:55,-0.290)  -- (axis cs:55, 0.090);
\draw[thin, gray] (axis cs:61,-0.290)  -- (axis cs:61, 0.090);
\draw[thin, gray, dotted] (axis cs:0, -0.290) -- (axis cs:0, 0.090);
\draw[thin, gray, dotted] (axis cs:30,-0.290) -- (axis cs:30, 0.090);
...
\end{axis}
\end{tikzpicture}
\caption{Confounder escalation for New York. The horizontal axis is the cumulative dimensionality of the adjustment set, rising from zero with no controls through six escalations to the full sixty-one-feature specification, and the vertical axis is the binarized doubly-robust ATE on log price for the leading sentence-BERT principal component, with the dashed horizontal line marking the null. Filled circles are point estimates and solid gray whiskers are ninety-five percent bootstrap intervals at $B = 500$ resamples. Two whiskers are clipped at the plot edges for legibility: the no-controls interval extends to $[-3.61, +2.66]$ and the zip-only interval to $[-3.27, +2.80]$, both shown as dotted segments at the axis limits. The coordinate block at $d = 32$ does almost all of the magnitude-shrinkage work, collapsing the point estimate from $-0.228$ to $-0.038$ and tightening the confidence band by roughly a factor of three. The monotone-shrinkage pattern is the Cinelli--Hazlett signature of confounding bias being removed as the spatial adjustment set is enriched.}
