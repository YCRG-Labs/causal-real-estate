...
\begin{abstract}
Does the language of a property listing raise its price because it carries
information about the property, or because it encodes the neighborhood the agent
already knows? Automated valuation models increasingly price homes by reading the
prose of a listing alongside its structured attributes, and the language reliably
improves their predictions, but whether that improvement reflects the property or
merely the neighborhood an agent's description encodes has not been tested, and
the distinction matters: a model that prices laundered geography offers nothing
beyond location and inherits its biases, while one that prices semantic content
adds real information. We treat the leading direction of a listing embedding as a
treatment and estimate its effect on log price with a partially linear double
machine learning score, holding the structured attributes and the parcel's
location fixed. Across twelve metropolitan markets the price effect of listing
language is almost unmoved by spatial adjustment in eleven; the exception is New
York, whose text is the most geographic of the twelve. The effect also holds on
realized sale prices, re-estimated across ten markets on a panel of nearly three
hundred thousand recorded transactions rather than the asking price an agent sets,
and is individually significant in every one. The estimates are associational, and
we bound their fragility with a sensitivity analysis. In most markets, a listing's language is priced for
its content, not its geography.
\end{abstract}
