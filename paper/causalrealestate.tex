...
\begin{abstract}
NLP-enhanced real estate models report that text embeddings of property descriptions add 0.85--0.88 to $R^2$ beyond geographic controls. We test whether this is causal or spatial confounding on 556,636 parcels across Boston, New York, and San Francisco, using a corpus of 1,044 unique Redfin listings (deduplicated to one row per URL from 2,982 scraper requests). Our gold-standard test is Double Machine Learning on SF: the leading text principal component has causal effect $\hat{\theta} = -0.001$ per $\sigma$ (95\% bootstrap CI $[-0.046, +0.051]$). For NYC, a confounder escalation test shows the estimate shrinking 76.5\% toward zero as spatial controls grow richer. A frozen-encoder probe recovers zip code at 8--18$\times$ random from adversarially ``deconfounded'' representations in all three cities; we formalize this as a consistent estimator of the discriminator-probe gap in $\mathcal{V}$-information. A 222-dimensional purely stylometric feature vector recovers zip at 3--4$\times$ random, well below the full transformer at 11--14$\times$: sociolinguistic register carries part of the spatial signal, but content features supply the larger share. Replicating Baur (2023), the BERT-augmented predictive gain evaporates under DML; replicating Shen (2021), a TF-IDF ``uniqueness'' measure retains a small distinguishable effect ($+0.072$ per $\sigma$, CI $[+0.028, +0.116]$). The text-price relationship is not uniformly null: its causal component is concentrated in a low-dimensional residual subspace orthogonal to the dominant location-confounded axis.
\end{abstract}
...
Our central finding refines this hypothesis rather than confirming it wholesale. Along the dominant axis of semantic variation, where the leading principal component of the embedding carries most of the signal, the causal effect of text on price is indistinguishable from zero once location is adjusted for: our gold-standard Double Machine Learning estimate on San Francisco, the only city with property-level geocoding, has a confidence interval that comfortably brackets zero. The confounding is acute and sociolinguistic in nature, because a purely stylometric feature vector containing no content words recovers zip code nearly as well as the full transformer, and a frozen-encoder probe shows that adversarial deconfounding leaves location almost entirely intact. The null is not total, however. A residual axis orthogonal to the location-confounded subspace, operationalized through the spatial-residual uniqueness measure of \citet{shen2021information}, retains a small but statistically distinguishable effect. The text-price relationship is therefore dominated by spatial confounding, with a modest genuine semantic component concentrated in a low-dimensional residual subspace.
...
Third, we quantify location confounding in text embeddings through mutual information, location-classification accuracy, and spatial autocorrelation, and we introduce a frozen-encoder probe diagnostic for adversarial deconfounding. We characterize this diagnostic theoretically as a consistent estimator of the discriminator-probe gap in $\mathcal{V}$-information, establishing that a discriminator driven to chance during gradient-reversal training can leave essentially all location information recoverable to a higher-capacity probe.

Fourth, we estimate the causal effect of semantic content with a Double Machine Learning estimator on the leading text principal component, finding no effect on price in San Francisco under full geocoded adjustment, and we trace the confounding mechanism to sociolinguistic register, since a purely stylometric feature vector recovers location nearly as well as the full embedding. Replicating \citet{shen2021information} and \citet{baur2023automated} on our data, we show that this null holds along the dominant semantic axis while a residual spatial-residual axis retains a small but distinguishable effect, so the text-price relationship is dominated by, but not reducible to, spatial confounding.
...
Concretely, we project the text embedding onto its first principal component (PC1, $z$-scored), partial out the full 35-feature confounder set from both PC1 and log-price using cross-fitted gradient boosting (5 folds), and regress the residualized outcome on the residualized treatment. The Neyman-orthogonal influence function gives closed-form standard errors and a minimum detectable effect (MDE) at 80\% power for two-sided $\alpha = 0.05$. A finite-sample calibration study of this estimator (Appendix~\ref{app:simulation}) shows that the influence-function interval is mildly anticonservative at our sample size; we therefore report as the headline a percentile-bootstrap interval that resamples the entire pipeline, including the principal-component construction, and recovers nominal coverage.

\textbf{San Francisco (primary estimate).} San Francisco is the only city in our dataset with property-level geocoding: the listing scraper captured individual latitude-longitude coordinates for all 348 unique descriptions, enabling a spatial join to the nearest parcel with full 34-covariate adjustment (continuous lat/lon, block-group census demographics, localized crime counts, amenity metrics, and parcel-level micro-geographic distances). The DML continuous estimator gives:
\begin{equation}
\hat{\theta}_{\text{SF}} = -0.001 \quad \text{(95\% bootstrap CI } [-0.046, +0.051]\text{, MDE } \pm 0.075\text{)}
\end{equation}
The confidence interval comfortably contains zero: we can rule out per-$\sigma$ effects exceeding roughly eight percent of price. This is the central causal result of the paper and the gold-standard estimate given the data quality hierarchy across our three cities.
...
Our results call for a reassessment of how the field has interpreted predictive performance in text-based valuation models. The strong correlations reported in prior work are not artifacts of poor methodology; they are real at the associational level. The error lies in the causal attribution. Text predicts price for the same reason that a thermometer predicts the season: both are downstream effects of a common cause, not independent sources of information. This reassessment is dominant rather than total. The bulk of the predictive signal, carried by the leading semantic axis, is spatially confounded; but our replication of \citet{shen2021information} (Section~\ref{sec:replications}) shows that a residual axis, defined as a listing's lexical deviation from its spatial neighbors, retains a small genuine effect of roughly five to ten percent per standard deviation. The substantive content of this residual axis is most plausibly property-specific information that warrants describing a listing unusually for its neighborhood, such as genuinely distinctive features or atypical condition and renovation. We cannot rule out, however, that it reflects residual confounding by listing or marketing effort correlated with unobserved quality that our covariates do not capture. Either reading leaves the headline intact, since the dominant axis is confounded and any genuine semantic component is small and confined to this residual subspace; the thermometer analogy holds for the dominant axis of variation, not for every dimension of the embedding.
