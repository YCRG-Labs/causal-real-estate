...
\item[(c)] There exist data distributions and class pairs $\Phi \subsetneq \Phi'$ such that for every saddle $(\phi^\star, \psi^\star)$ of the gradient-reversal game restricted to $\Phi$, $V_\Phi(Z_{\phi^\star}; C) = 0$ but $V_{\Phi'}(Z_{\phi^\star}; C) \ge H(C) - \delta$ for arbitrary $\delta > 0$.
\end{enumerate}
\end{proposition}

Statement (a) is folklore in the variational MI literature: it appears as Proposition 1 of \citet{xu2020theory} in the language of $\mathcal{V}$-information and as the central observation of \citet{pimentel2020information} in the language of probe capacity. We restate it for self-containedness. Statements (b) and (c) are our contribution. Part (b) connects (a) to the gradient-reversal training objective and formalizes the post-hoc adversary used as a heuristic by~\citet{moyer2018invariant} and~\citet{elazar2018adversarial} as a consistent statistical estimator. Part (c) establishes that gradient-reversal can reach the optimum of its own minimax game while leaving essentially all of the mutual information recoverable to a richer probe; in spirit this parallels saddle-quality results for linear adversarial erasure, but the construction we use is XOR-style and yields a sharp $H(C)$ gap. Proofs are deferred to Appendix~\ref{app:frozen_probe_proofs}; Appendix also reports three controlled numerical experiments verifying each part on synthetic data.
...
We exhibit a data distribution and class pair $\Phi \subsetneq \Phi'$ for which the gap at every saddle equals $H(C)$ minus a vanishing remainder.

\textit{Construction.} Take $X = (X_1, X_2) \in \{0, 1\}^2$ with $X_1, X_2 \overset{\text{iid}}{\sim} \text{Bernoulli}(1/2)$, and $C = X_1 \oplus X_2$ (the XOR of the two binary features). Then $C \in \{0, 1\}$ uniformly with $H(C) = \log 2$.

Let $\Phi$ be the class of linear logistic-regression classifiers $D_\psi(z) = \sigma(w^\top z + b)$ on $z \in \mathbb{R}^2$, and let $\Phi'$ be any class containing a $2$-layer MLP with at least one hidden unit (e.g., $D_{\psi'}(z) = \sigma(W_2 \, \mathrm{ReLU}(W_1 z + b_1) + b_2)$ with hidden width $\ge 2$).

Take $E_\phi$ as the identity (or any orthogonal transformation; the argument is invariant). Then on the joint distribution $(Z, C)$:
\begin{itemize}
\item For any $D_\psi \in \Phi$ (linear), the conditional posterior $\Pr[C = 1 \mid Z]$ takes value $1$ on $\{(0,1), (1,0)\}$ and value $0$ on $\{(0,0), (1,1)\}$, which is not linearly separable. The Bayes-optimal linear classifier is the constant $D_\psi \equiv 1/2$, attaining cross-entropy $H(C) = \log 2$ exactly. Therefore $V_\Phi(Z; C) = 0$, and every linear-discriminator saddle $(\phi^\star, \psi^\star)$ of the gradient-reversal game restricted to $\Phi$ has the same value.
\item For $\Phi'$ containing the $2$-layer MLP, the Bayes-optimal posterior $\Pr[C = 1 \mid Z] \in \{0, 1\}$ is exactly representable. Hence $V_{\Phi'}(Z; C) = I(Z; C) = H(C) = \log 2$.
\end{itemize}

The gap at the saddle is therefore exactly $V_{\Phi'} - V_\Phi = \log 2 - 0 = H(C)$. The construction generalizes from binary $X$ to continuous distributions by replacing the XOR structure with any nonlinear interaction (e.g., $C = \mathrm{sign}(X_1 X_2)$ for Gaussian $X_1, X_2$), at the cost of a vanishing $O(\delta)$ remainder controlling the off-diagonal probability mass. \qed
...
Proposition~2(c) (XOR saddle-gap construction) & Wraps \texttt{frozen\_probe\_gap.py}; assertions on $V_\Phi$, $V_{\Phi'}$ & PyTorch \\
\bottomrule
\end{tabular}
