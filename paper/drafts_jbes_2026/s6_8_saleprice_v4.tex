...
cleared. This does not sever the simultaneity outright, since a realized sale price
anchors on the list price the same agent set \citep{genesove2001loss}, but it
replaces the agent's stated number with a market-tested one and so weakens the
channel that a purely asking-price outcome leaves open. We assemble a panel of recorded sales by collecting sold listings across
...
sale date and the structural attributes. After we restrict to residential
single-unit dwellings with a usable description, a recorded price, and a geocode,
the panel holds roughly one hundred ninety thousand transactions closed between
2023 and 2026, close to three times the pooled description corpus used
...
a product of how the text measure is discovered and estimated on the same data.
...
the listing-text association is a feature of market valuation rather than a quirk
