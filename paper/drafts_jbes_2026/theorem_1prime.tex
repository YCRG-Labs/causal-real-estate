...
so $D(v_{\mathrm{PC}})\to1$ and $E(v_{\mathrm{PC}})\to0$, and by \eqref{eq:thetav}
$\theta(v_{\mathrm{PC}})\to\theta+\beta b$, the fully geography-confounded estimand in
which the confounding term $\beta b$ swamps the semantic effect $\theta$. When
$\beta b$ is opposite in sign to $\theta$ and larger in magnitude, the reported effect
is sign-reversed: the default summary of the text returns a coefficient of the wrong
sign.
\end{corollary}
