...
explicit about that reading throughout. That we treat text as the causal variable
rather than as a confounder to adjust for places the paper on the treatment side of
the causal-text program surveyed by \citet{keith2020text}, whose adjustment strand
runs from the text-matching of \citet{roberts2020adjusting} to the
linguistic-property effects of \citet{pryzant2021causal}. A second lesson from this
literature bears on estimation rather than interpretation, since \citet{egami2022texts} establish
that when the treatment representation is learned from the documents its estimation
error must not contaminate the outcome stage. We take that concern up in the
calibration of Appendix~\ref{app:simulation}, where forming the direction on the
pooled corpus rather than within each market shrinks the channel, an out-of-fold
projection removes most of what remains, and a full-pipeline bootstrap carries the
residual into the reported intervals.
...
\citet{wangblei2019}, and \citet{ogburn2019comment} show that such a stand-in,
...
erasure and adjustment alone but escalates to the within-building and twin-unit
