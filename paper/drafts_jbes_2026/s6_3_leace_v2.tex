...
\paragraph{Downstream effect on the Baur DML estimate.} The Baur PC1
estimates in Table~\ref{tab:baur-12city} are reported on the
unprojected BERT embedding, consistent with the published Baur,
Rosenfelder, and Lutz protocol. The post-LEACE+IGBP deconfounding
machinery developed here serves a different purpose: it certifies
that the encoder representation in our analysis carries no remaining
linear or near-linear leakage of the lat-lon concept by the end of
the IGBP iteration. This certification is a property of the encoder
and the deconfounding pipeline, not of the per-market DML estimate,
and it transfers to any downstream analysis that wants to use the
projected embedding as a treatment.
