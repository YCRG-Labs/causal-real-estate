...
the regime where the propensity learner is already flexible. The severe
miscoverage we observe under the linear-classifier panel does not recur in
the applied diagnostics of Appendix~\ref{app:simulation}, where the
influence-function interval is at most mildly anticonservative at the null,
on the order of ten to fifteen percent in the standard error, and we report
bootstrap intervals as the remedy for that residual narrowing rather than
treat the applied coverage as exactly nominal.
