...
the partial strengths $r_D$ and $r_Y$ that enter the \citet{cinelli2020making} and
\citet{chernozhukov2022long} bias bound converge to the raw marginal strengths
$R^2_{T \sim C}$ and $R^2_{Y \sim C \mid T}$, so the shrinkage that conditioning on
$X$ applies to a confounder correlated with the controls is absent; and (ii) the
...
The argument is short. Partialling $X$ out of $T$ and of $C$ leaves residuals
whose correlation, by the recursive partial-correlation identity, tends to the raw
correlation $\mathrm{Cor}(T, C)$ as $\mathrm{Cor}(C, X) \to 0$, because the term
that subtracts the variance $C$ shares with $X$ vanishes and the normalizing factor
tends to one; the same holds on the outcome side, which gives part (i). Part (ii)
