...
residuals of $T$, $C$, and $Y$ after partialling out the full control vector $X$ by
...
correlations among these residuals. As $R^2(C \mid X) \to 0$ the residual $\tilde C$
converges to $C$ centered, because $X$ carries no linear information about $C$ to
remove, so the confounder's residual variation is undiminished by conditioning and
$r_D \to R^2_{T \sim C}/(1 - R^2_{T \sim X})$, no smaller than its raw counterpart;
...
identity $\rho_{TC \mid X} = (\rho_{TC} - \rho_{TX}\rho_{CX}) /
\sqrt{(1 - \rho_{TX}^2)(1 - \rho_{CX}^2)}$, in which the protective subtraction
$\rho_{TX}\rho_{CX}$ vanishes with $\mathrm{Cor}(C, X)$; the high-dimensional case
...
$R^2_{T \sim C}/(1 - R^2_{T \sim X})$ with $R^2_{T \sim X}$ the multiple coefficient
...
bias rather than merely bounding it, so the bound the audit reasons about is tight;
the verification is released with the replication code. Part (ii)
...
not that the formal bound cannot detect $C$; the \citet{cinelli2020making} bound,
...
predictable from the treatment, $\mathrm{AUC}(C \mid T) > 0.75$, the near-separation
