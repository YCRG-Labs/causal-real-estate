...
embeddings on log sale price across twelve metropolitan areas spanning
the major U.S. housing-market typologies, using \nlistingsTotal{}
deduplicated Redfin listings matched to \nparcels{} parcels.
...
in San Francisco of \shenSFpoint{} (cluster-$t$ CI \shenSFci{}) within
$0.7$ standard errors of the published Atlanta point estimate, while
the leading sentence-BERT principal component identifies a per-$\sigma$
effect in New York of \baurNYpoint{} (\baurNYci{}); the counterfactual
intervention recovers a pooled total effect on log price of
\cfTEpooled{} (\cfTEci{}) that excludes zero in eleven of twelve
cities and exhibits strong cross-LLM concordance ($\rho = 0.96$,
sign-agreement $10/12$). A REML mixed-effects meta-regression with
Knapp-Hartung small-sample adjustment identifies the renter share as
the single significant moderator of cross-city heterogeneity
($\beta = \metaRenterCoef{}$, $p = 0.031$, $R^2_{\mathrm{meta}} =
\metaR2{}$), and an omnibus joint-null Wald test rejects homogeneity
