...
The argument combines a spectral perturbation bound for the estimated direction with
the orthogonal-score expansion of the fixed-direction estimator. Under the spectral gap
of Assumption~\ref{as:ed-gap} the estimated direction converges at rate $N^{-1/2}$,
with the bilinear normal approximation of \citet{koltchinskii2016asymptotics} supplying
the distribution and the deterministic subspace bound of \citet{yu2015useful} the rate.
The rate comparison of Assumption~\ref{as:ed-rate}, together with the out-of-fold
construction that makes $\hat v$ independent of the market-$k$ score, renders the linear
direction term $o_p(n_k^{-1/2})$ and admits no covariance into the limiting variance.
With the direction held at $v^\ast$ the cross-fitted Robinson estimator is
asymptotically linear, the nuisance error entering at second order by Neyman
orthogonality \citep{chernozhukov2018double}, and its sum over the spatially dependent
field is asymptotically normal by \citet{jenish2009central} with variance estimated by
the spatial-HAC estimator. Appendix~\ref{app:estimated-direction-proof} carries out each
step in full.
...
Together the two theorems fill in the identification the controls promised in
Section~\ref{sec:identification}: Theorem~\ref{thm:idprime} names the direction on
which observed location is removed exactly, and
Theorem~\ref{thm:estimated-direction} supplies the inference once that direction is
estimated. Both concern identification against observed location. The residual the
designs cannot reach, unmeasured interior quality and the nonlinear leakage the linear
erasure leaves behind, stays with the sensitivity bound of
Section~\ref{sec:sensitivity-12} rather than with these theorems.
