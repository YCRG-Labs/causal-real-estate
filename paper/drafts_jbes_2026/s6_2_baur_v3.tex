...
axes and delivers a single direction of effect across the panel. Ten of the
twelve markets return a confidence interval that excludes zero at the $95\%$
level, and eleven of the twelve carry a positive point estimate. New York is the
sole negative, at $-0.009$ on an interval $[-0.022, +0.004]$ that covers zero,
and Seattle is the one other market whose interval covers zero, at $+0.030$ on
$[-0.002, +0.062]$. Philadelphia and Chicago anchor the high end at $+0.374$ on
$[+0.356, +0.392]$ and $+0.358$ on $[+0.322, +0.394]$, followed by the District
of Columbia at $+0.151$ and Dallas at $+0.142$. Atlanta and San Francisco sit
near $+0.11$, the latter on $[+0.056, +0.155]$, and Portland, Boston, and Denver
fall between $+0.061$ and $+0.076$, while Phoenix and Seattle return the smallest
positive effects at $+0.028$ and $+0.030$. Portland, which the per-market PCA had
rendered a degenerate outlier, now sits squarely inside the panel at $+0.076$ on
$[+0.051, +0.101]$, so the panel is clean at twelve markets and there is no
...
convention (text). Ten of the twelve intervals exclude zero; New York and Seattle
cover it.}
...
Phoenix       & $7{,}087$  & $+0.028$ & $[+0.010, +0.045]$ \\
New York      & $15{,}255$ & $-0.009$ & $[-0.022, +0.004]$ \\
\midrule
Pooled (PM, HKSJ) & & $+0.125$ & $[+0.047, +0.202]$ \\
\bottomrule
\end{tabular}
\end{table}
...
per-standard-deviation effect of $+0.125$ with standard error $0.035$, a $t$
statistic of $+3.55$ on $11$ degrees of freedom, a two-sided $p$ of $0.005$, and
a $95\%$ confidence interval of $[+0.047, +0.202]$. The pooled estimate is
detectable and positive across the panel under the adopted orientation.
Heterogeneity is large, with an $I^2$ near $97\%$ and a Paule-Mandel $\tau^2$ of
about $0.015$, so the pooled point estimate masks substantial cross-market
dispersion that Philadelphia and Chicago anchor at one end and New York at the
...
effect of $+0.125$ on log listing price is about a thirteen percent change in price,
...
pooled text-axis effect is more than three times the marginal price of an additional
...
uniqueness effect of $+0.114$ and a BERT-PC1 effect of $+0.106$, so where both
are measured on the same listings they agree. Pooled across the twelve markets,
however, the two channels part company. The sentence-BERT axis holds at $+0.125$,
while the uniqueness effect collapses to a pooled value indistinguishable from
zero once the full confounder block replaces the coordinate-only controls that
the superseded panel had used for nine of the twelve markets. We read this
divergence as a discriminating result rather than a failure. The two channels are
alternative encodings of the same listing text, but they encode different things:
the leading sentence-BERT component recovers a utilitarian-to-amenity axis of
semantic content, whereas the Shen score measures how textually distinctive a
description is relative to its neighbors, a quantity that is itself largely a
function of neighborhood. That the semantic-content axis prices net of the
neighborhood controls while the distinctiveness score does not is consistent with
the paper's central claim, that it is the content of the language and not the
geography it encodes that carries the effect.
