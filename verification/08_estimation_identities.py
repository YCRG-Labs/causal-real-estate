...
def check_gelbach(seed=0):
    ...
    th_short = ols(np.column_stack([T, W]), Y)[1]
    full = ols(np.column_stack([T, W, G]), Y)
    th_long = full[1]
    delta = th_short - th_long
    contrib = 0.0
    for j in range(G.shape[1]):
        delta_g = full[1 + 1 + W.shape[1] + j]
        gamma_g = ols(np.column_stack([T, W]), G[:, j])[1]
...
def check_delta_method():
    ...
    delta = th * D
    grad = sp.Matrix([sp.diff(delta, th), sp.diff(delta, D)])
    Sigma = sp.diag(vth, vD)
